...
import { faUsers, faBoxOpen, faMoneyCheckAlt, faSignOutAlt } from '@fortawesome/free-solid-svg-icons';
...
const PaginaInicio = () => {
    ...
    return (
        <div className="flex h-full">
            ...
            {/* Línea separadora */}
            <div className="fixed bottom-12 left-0 right-0 h-px bg-gray-300 md:hidden"></div>
            ...
            {!isKeyboardOpen && (
                <div className={`fixed bottom-0 left-0 right-0 z-50 py-1 bg-gray-100 text-black text-center text-base font-bold md:hidden flex justify-around`}>
                    ...
                    <Link to="/logout" className="flex flex-col items-center group">
                        <FontAwesomeIcon icon={faSignOutAlt} size="lg" className="text-black mb-1 group-hover:text-lime-500 group-hover:stroke-black group-hover:stroke-1 transition-all duration-200 transform group-hover:-translate-y-1" />
                        <span className="text-xs text-black">Cerrar sesión</span>
                    </Link>
                </div>
            )}
